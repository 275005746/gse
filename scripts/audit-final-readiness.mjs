...
const matrix = [
  ...
  { area: 'npm tarball install', status: exists('package.json') && exists('scripts/audit-npm-tarball-install.mjs') && npmTarballInstallAudit.includes('npm pack creates one tarball') && npmTarballInstallAudit.includes('installed gse bin renders help from the installed package') && npmTarballInstallAudit.includes('installed init preview is read-only for a separate consumer project') && npmTarballInstallAudit.includes('installed init execute creates project state and preserves project rules') && npmTarballInstallAudit.includes('installed bin runs status against the consumer project') && npmTarballInstallAudit.includes('installed README audit passes') ? 'verified' : 'missing', evidence: 'scripts/audit-npm-tarball-install.mjs verifies npm tarball creation, clean consumer install, installed help/init/status execution, and installed README audit' },
...
const checks = [
  ...
  check('FR07', 'install and publish rows include CLI proof boundaries', finalReadiness.includes('installed short CLI status command') && finalReadiness.includes('installed `gse` bin execution') && finalReadiness.includes('CLI bin metadata') && finalReadiness.includes('URL-installed short CLI status command') && distributionAudit.includes('installed short CLI wrapper runs status command') && npmTarballInstallAudit.includes('installed init execute creates project state and preserves project rules') && npmTarballInstallAudit.includes('installed bin runs status against the consumer project') && npmPublishDryRunAudit.includes('publish dry-run preserves CLI package metadata before publication') && remoteDistributionAudit.includes('remote installed short CLI wrapper runs status command'), 'references/final-readiness.md, distribution audits, publish dry-run audit'),
