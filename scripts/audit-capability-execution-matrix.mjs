...
const taskLevels = read('references/task-levels.md')
const stageOrchestrator = read('references/stage-orchestrator.md')
const contextOrchestration = read('references/context-orchestration.md')
const hostAdapters = read('references/host-adapters.md')
...
const requiredCapabilities = [
  ...
  'Capability-adaptive procedural autonomy',
...
const checks = [
  ...
  check('CEM13', 'procedural autonomy boundary is documented across task, stage, context, and host contracts', [taskLevels, stageOrchestrator, contextOrchestration, hostAdapters].every((text) => /autonom/i.test(text)) && taskLevels.includes('External and destructive operations') && stageOrchestrator.includes('false completion claim') && contextOrchestration.includes('Model identity') && hostAdapters.includes('does not set `canAutoContinue`'), 'task-levels, stage-orchestrator, context-orchestration, host-adapters'),
]
