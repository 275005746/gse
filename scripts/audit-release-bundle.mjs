...
const canonicalBundleReleaseOwnerActionPlan = read('.gse/release-bundles/gse-release-bundle-v1.0.0/release-owner-action-plan.md')
const canonicalBundlePublicReleaseChecklist = read('.gse/release-bundles/gse-release-bundle-v1.0.0/public-release-checklist.md')
const canonicalBundleKitReleaseOwnerActionPlan = read('.gse/release-bundles/gse-release-bundle-v1.0.0/owner-external-gate-kit/release-owner-action-plan.md')
const canonicalBundleManifestData = parseJson(read('.gse/release-bundles/gse-release-bundle-v1.0.0/bundle-manifest.json'))
...
const checks = [
  ...
  check('RB06v', 'bundle manifest records compact validation check details', Array.isArray(bundleManifestData?.validationChecks) && bundleManifestData.validationChecks.length > 0 && bundleManifestData.validationChecks.some((item) => item.command?.includes('audit-command-execution.mjs')) && summary.includes('Validation manifest detail'), 'bundle-manifest.json, release-summary.md'),
  check('RB06v2', 'canonical bundle manifest records current validation check details', Array.isArray(canonicalBundleManifestData?.validationChecks) && canonicalBundleManifestData.validationChecks.length > 0 && canonicalBundleManifestData.validationChecks.some((item) => item.command?.includes('audit-command-execution.mjs') || item.command?.includes('audit-evidence-placeholders.mjs')), '.gse/release-bundles/gse-release-bundle-v1.0.0/bundle-manifest.json'),
  ...
  check('RB06e5', 'canonical release bundle checklist matches pending gate count', canonicalBundlePublicReleaseChecklist.includes('GSE Public Release Checklist') && canonicalBundlePublicReleaseChecklist.includes('Pending owner/external gates: ' + (canonicalReleaseStatusManifest?.publicAcceptance?.pendingGates?.length ?? 0)), '.gse/release-bundles/gse-release-bundle-v1.0.0/public-release-checklist.md'),
