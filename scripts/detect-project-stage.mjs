...
const requestedTaskProfile = readArg('--task-profile', null)
...
const projectMode = state?.mode ?? null
const taskProfile = resolveTaskProfile({
  intent,
  projectMode,
  preferredProfile: requestedTaskProfile,
})
const autonomyPolicy = resolveAutonomyPolicy({ taskProfile })
const boundedMaintenanceIntent = /\b(fix|bug|typo|copy|css|readme|docs?|narrow|local|small|rename)\b|修复|文案|样式|局部|小改|重命名/i.test(intent)
const boundedLiteMaintenance = taskProfile.status === 'proceed'
  && taskProfile.taskProfile === 'lite'
  && implementationEvidence
  && boundedMaintenanceIntent
...
stageBasis.push(`project mode ${projectMode ?? 'unspecified'} is separate from task profile ${taskProfile.taskProfile}`)
if (taskProfile.triggerIds.length) stageBasis.push(`task profile triggers: ${taskProfile.triggerIds.join(', ')}`)
...
if (taskProfile.status === 'ask_user') {
  detectedStage = taskProfile.taskProfile === 'enterprise' ? 'architecture' : 'requirements'
  detectedDecision = 'ask_user'
  missingArtifacts.push('resolve the task hard-risk classification before implementation')
} else if (boundedLiteMaintenance) {
  detectedStage = testFiles.length ? 'verification' : 'implementation'
  if (!testFiles.length) missingArtifacts.push('focused verification for the bounded maintenance task')
...
const report = {
  ...
  task_profile_status: taskProfile.status,
  task_profile_reason: taskProfile.reasonCode,
  task_profile_triggers: taskProfile.triggerIds,
